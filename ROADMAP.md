...
- [ ] Add `device` and `o` driver opts using env vars so Komodo can inject them per host
- [ ] Verify volumes work with `docker compose up` from a clean clone
...
- [ ] Pick one stack (e.g. `media`) to own and create the network
- [ ] Keep `external: true` on the other two (`arr`, `downloader`)
- [ ] Document the deploy order dependency

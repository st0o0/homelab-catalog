...
1. Restores the age key from Bitwarden if one exists there, otherwise
   generates a new one at `~/.config/sops/age/keys.txt`
2. Backs the key up to Bitwarden (secure note "Homelab Catalog SOPS Age
...
just secrets nas-01     # edit per-host secrets (komodo/hosts/nas-01/secrets.sops.yaml)
```
...
1. `just secrets [TARGET]` — edit the value, save, it's re-encrypted automatically
2. Commit and push:
   ```bash
...
`just secrets <hostname>` copies `komodo/hosts/secrets.sops.yaml.tpl` into
